...
def test_verify():
    pr = PatternReplacer(samplepattern_no, '-/')
    assert pr.verify('i-want-to-/a/-g') == True
    assert pr.verify('i-wan-tto-/a/-g') == True
    assert pr.verify('i-wan-tto-/ag/-') == False
    assert pr.verify('-iwan-tto-/a/-g') == False
    assert pr.verify('i-wan-t-to-/a-g') == False
    assert pr.verify('/i-want-to-/ag') == False
    assert pr.verify('/i-wantto-/ag/') == False
